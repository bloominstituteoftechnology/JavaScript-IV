...
class Humanoid extends CharacterStats {
  constructor(data) {
    ...
    this.weapons = data.weapons.toString();
...
class Villian extends Humanoid {
  ...
  fireball(character) {
    if (character.hp > 1) {
      character.hp--;
      return `${this.name} casts fireball. ${character.takeDamage()}. ${
        character.name
      } now has ${character.hp} hp.`;
    } else {
      character.hp--;
      return `${this.name} casts fireball. ${character.takeDamage()}. ${
        character.name
      } now has ${character.hp} hp. ${character.destroy()}`;
    }
  }
}
...
class Hero extends Humanoid {
  ...
  smite(character) {
    if (character.hp > 1) {
      character.hp--;
      return `${this.name} casts smite. ${character.takeDamage()}. ${
        character.name
      } now has ${character.hp} hp.`;
    } else {
      character.hp--;
